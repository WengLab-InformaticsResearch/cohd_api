  ...
  function DomColumns(obj, settings) {
    ...
    this.add = function($column, position, skipAppend, skipUpdate) {
      var columns = settings.table.columns,
          label = $column.text(),
          // Casey's hack to get column "p-value" to work. I think dynatable is converting the column headings to camelCase
          // prior to saving them as keys, but maybe the JSON keys are not being changed.
          id = $column.data('dynatable-column') || label,
        ...
        for (attr in record) {
          if (record.hasOwnProperty(attr)) {
            ...
            if ((typeof(attrValue) === "string" && attrValue.toLowerCase().indexOf(queryValue.toLowerCase()) !== -1) || 
              (typeof(attrValue) === "number" && attrValue.toString().indexOf(queryValue) !== -1)) {
              contains = true;

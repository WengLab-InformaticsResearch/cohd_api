...
class CohdTrapi150(CohdTrapi):
    ...
    def _interpret_query(self):
        ...
        if self._query_edge_predicates is not None:
            ...
            for edge_predicate in self._query_edge_predicates:
                ...
                for pd in predicate_descendants:
                    if pd in CohdTrapi150.supported_edge_types:
                        edge_supported = True
                        break

                # Check directionality of predicate
                if edge_predicate in CohdTrapi150.edge_types_positive:
                    positive_edge = True
                elif edge_predicate in CohdTrapi150.edge_types_negative:
                    negative_edge = True
                else:
                    positive_edge = True
                    negative_edge = True
        ...
        if self._concept_1_qnode_categories is not None:
            self._concept_1_qnode_categories = CohdTrapi150._process_qnode_category(self._concept_1_qnode_categories)
            ...
            for supported_cat in CohdTrapi150.supported_categories:
        ...
        if self._concept_2_qnode_categories is not None:
            self._concept_2_qnode_categories = CohdTrapi150._process_qnode_category(self._concept_2_qnode_categories)
    ...
    def _initialize_trapi_response(self):
        ...
        self._response = {
            # From TRAPI Extended
            'resource_id': CohdTrapi._INFORES_ID,
            'tool_version': CohdTrapi150.tool_version,
            'schema_version': CohdTrapi150.schema_version,
            'biolink_version': CohdTrapi150.biolink_version,
